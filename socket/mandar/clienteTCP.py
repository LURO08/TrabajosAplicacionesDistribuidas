...
class TCPClientApp:
    ...
    def start_client(self):
        ...
        self.thread = threading.Thread(target=self.connect_to_server, args=(host, port))
        self.thread.start()
    ...
    def receive_messages(self):
        while self.connected:
            try:
                response = self.client_socket.recv(1024) # type: ignore
                if not response:
    ...
    def send_message(self):
        ...
        if message:
            try:
                self.client_socket.sendall(message.encode()) # type: ignore
    ...
    def disconnect(self):
        if self.connected:
            try:
                self.client_socket.close() # type: ignore
            except socket.error as e:
